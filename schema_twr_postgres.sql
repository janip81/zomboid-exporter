-- zomboid-exporter Postgres schema -- ThoseWhoRemain (TWR) quest engine.
-- Applied ONLY when --twr-enabled=true (see main.go and
-- zomboid-exporter-ideas/antagonist/design/exporter-twr-feature-
-- boundary.md for the split rationale, 2026-08-14). A plain stats-only
-- exporter deployment must never see any of these tables just because
-- Postgres happens to be configured -- see schema_stats_postgres.sql
-- for what's always applied.

-- ThoseWhoRemain (TWR) mod result tracking -- see
-- zomboid-exporter-ideas/antagonist/spawn-result-tracking.md for the
-- design decision this implements. Separate from `events` (stats
-- schema) deliberately: these are control-plane/audit records of world
